...
def getFlowReps(dataset_name, max_flow_length):
    ...
    if dataset_name == "unibs":
        packet_flow_reps = loadFlows(path= "data/unibs/unibsPacketRep.json", cls= PacketFlowRepressentation)
        for flow_rep in packet_flow_reps:
            flow_rep.class_type = assignUNibsClass(class_type= flow_rep.class_type)
    elif dataset_name.lower() == "VNAT".lower():
        packet_flow_reps = loadFlows(path= "data/VNAT/flowStore/vnatPacketRep.json", cls= PacketFlowRepressentation)#VNATDataFrameProcessor.getPacketFlows()
        packet_flow_reps = VNATDataFrameProcessor.convertLabelsToTopLevel(flows= packet_flow_reps)
    elif dataset_name.lower() == "UTMobileNet".lower():
        packet_flow_reps = loadFlows(path= "data/UTMobileNet2021/mobilenetPacketRep.json", cls= PacketFlowRepressentation)
        ...
        packet_flow_reps = loadFlows(path= "data/MIRAGE-2019/miragePacketRepApp.json", cls= PacketFlowRepressentation)
    ...
    timeslot_flow_reps  = Parallel(n_jobs=18)(delayed(convertPacketRepToTimeslotRepEffecient)(flow, .001, [1250]) for flow in packet_flow_reps)
...
def getTrainTestOODBalanced(dataset_name, test_size,ood_classes = [], packet_max_length = 15, timeslot_max_length= 15, max_flow_length_in_seconds = 90):
    ...
    indices = filterBasedOnLength(reps= timeslot_flows,min_length= 8)
...
def readTrainTestOODFlows(base_path, dataset_name):
    ...
    if os.path.exists(os.path.join(dir_path,"ood_packet_flows.pkl")):
        ood_packet_flows = loadFlows(path= os.path.join(dir_path,"ood_packet_flows"), cls= PacketFlowRepressentation)
        ood_timeslot_flows = loadFlows(path= os.path.join(dir_path,"ood_timeslot_flows"), cls= TimeslotRepresentation)
